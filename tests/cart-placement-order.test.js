...
const clearCall = source.indexOf('await clearVendorCartInTab(tab.id, job.retailer)', runStart);
const clearMessage = source.indexOf("{ type: 'CLEAR_VENDOR_CART', retailer }", runStart);
const productLoop = source.indexOf('for (let index = 0; index < items.length; index++)', runStart);
const quantityCall = source.indexOf('await setCartQuantitiesInTab(tab.id, items, job.retailer)', productLoop);
...
assert.match(source, /addProductInTab\(tab\.id, \{ \.\.\.item, quantity: 1 \}, job\.retailer\)/);
assert.match(source, /CheckoutCartDisplayView/);
assert.match(source, /retailer=\$\{encodeURIComponent\(job\.retailer\)\}/);
